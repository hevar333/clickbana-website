...
import { setupArabicKeyboardMapping } from './arabicKeyboardMapping.js';
...
import initContextMenuSearch from './contextMenuSearch.js';
  ...
  // Initialize Arabic keyboard mapping
  const arabicKeyboard = setupArabicKeyboardMapping(editor, showToast);
  
  // Set initial state from toggle
  arabicKeyboard.setEnabled(kbToggle.checked);
  
  // Connect toggle button
  kbToggle.addEventListener('change', (e) => {
    arabicKeyboard.setEnabled(e.target.checked);
  });
  
  // Optional cleanup
  // window.addEventListener('beforeunload', () => arabicKeyboard.cleanup());
});
    ...
    function toggleShiftState() {
      ...
      updateKeyLabels();
      showToast(isShifted ? "تم تفعيل شيفت" : "تم إلغاء تفعيل شيفت");
    }
    ...
    function toggleCapsLock() {
      ...
      updateStateIndicators();
      updateKeyLabels();
      showToast(isCapsLock ? "تم تفعيل كابس لوك" : "تم إلغاء تفعيل كابس لوك");
    }
    ...
    function updateKeyLabels() {
      ...
      keys.forEach(key => {
        ...
        if (key.classList.contains('letter') && latinKey) {
          let arabicChar;
          if (isCapsLock) {
            arabicChar = isShifted ? dataKey : dataShift;
          } else {
            arabicChar = isShifted ? dataShift : dataKey;
          }
          
          key.innerHTML = `<div class="arabic-letter">${arabicChar}</div><div class="latin-key">${latinKey}</div>`;
        } else if (dataShift) {
    ...
    function setupLetterKeys() {
      ...
      letterKeys.forEach(key => {
        const arabicChar = key.getAttribute('data-key');
        const latinKey = key.getAttribute('data-latin');
        key.innerHTML = `<div class="arabic-letter">${arabicChar}</div><div class="latin-key">${latinKey}</div>`;
      });
    }
    ...
    function updateWordCount() {
      ...
      wordCountEl.textContent = `كلمات: ${words}`;
      charCountEl.textContent = `أحرف: ${characters}`;
    }

    // Link insertion
    function createLink() {
      const url = prompt('الرجاء إدخال الرابط:', 'https://');
      if (url) {
        const selection = window.getSelection();
        
        if (!selection.toString().trim()) {
          const linkText = prompt('الرجاء إدخال نص الرابط:', 'رابط');
          if (linkText) {
    ...
    function performSearch(engine) {
      ...
      if (selection) {
        ...
        showToast('الرجاء تحديد نص للبحث عنه');
      }
    }

    // Clear all text
    function clearAllText() {
      if (confirm('هل أنت متأكد من حذف كل النص؟')) {
        editor.innerHTML = '';
        editor.focus();
        updateWordCount();
        showToast('تم حذف النص بالكامل');
      }
    }
