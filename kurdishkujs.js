...
const editor = document.getElementById('text-input');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    kbToggle.addEventListener('change', (e) => {
      ...
      showToast(e.target.checked ? 
        "Klavyeya Kurdî çalak bû" : 
        "Klavyeya Kurdî neçalak e"
      );
    });
  ...
  function toggleShiftState() {
    ...
    updateKeyLabels();
    showToast(isShifted ? "Shift çalak kir" : "Shift neçalak kir");
  }
  ...
  function toggleCapsLock() {
    ...
    updateStateIndicators();
    updateKeyLabels();
    showToast(isCapsLock ? "Caps Lock çalak kir" : "Caps Lock neçalak kir");
  }
  ...
  function updateWordCount() {
    ...
    wordCountEl.textContent = `Peyv: ${words}`;
    charCountEl.textContent = `Tîp: ${characters}`;
  }

  // Link insertion
  function createLink() {
    const url = prompt('URL girêdanê binivîse:', 'https://');
    if (url) {
      const selection = window.getSelection();
      
      if (!selection.toString().trim()) {
        const linkText = prompt('Nivîsa girêdanê binivîse:', 'Girêdan');
        if (linkText) {
  ...
  function performSearch(engine) {
    ...
    if (selection) {
      ...
      showToast('Ji kerema xwe nivîsek hilbijêrin');
    }
  }

  // Clear all text
  function clearAllText() {
    if (confirm('Ma tu rastî dixwazî hemû nivîsê jê bibî?')) {
      editor.innerHTML = '';
      editor.focus();
      updateWordCount();
      showToast('Hemû nivîs hat jêbirin');
    }
  }
  ...
  function copyAllText() {
    ...
    try {
      ...
      if (successful) {
        showToast('Nivîs hat kopîkirin');
      } else {
        showToast('Çewtiyek çêbû');
      }
    } catch (err) {
      showToast('Çewtiyek çêbû');
    }
...
`);
        ...
        alert('Ji kerema xwe popupan rê bidin');
    }
}
